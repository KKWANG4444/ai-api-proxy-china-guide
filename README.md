...
这份指南面向需要在一个项目里使用多个模型的开发者。重点不是宣传“多少个模型”或给出一组无法复现的延迟数字，而是把接入、验证和排错流程讲清楚。
...
这里只列经过核验的样例，不把配置条目数量当成当前在线模型总数。豆包等模型如处于维护状态，也不能宣传为当前可用。
...
目录会变，配置存在也不代表在线。生产配置应保存精确模型 ID，并在部署前做真实请求。

### 单次延迟

没有时间、测试地区、样本量和 p50/p95 的“200ms”没有参考价值。请从自己的部署区域测试。
...
网关不应在用户不知情时把模型 A 换成模型 B。需要回退时，应由应用侧定义能力相近的模型组，并记录实际响应模型。
...
1. 请求头是否为 `Authorization: Bearer YOUR_KEY`；
2. Key 是否完整、启用；
3. 账户状态是否正常。
